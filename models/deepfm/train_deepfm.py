...
from rec_deepfm.model_deepfm import *
...
def run(param_dict):
    ...
    deepfm_model = DeepFM(param_dict)
    deepfm_model.compile(optimizer=get_optimizer(param_dict["optimizer"], learning_rate=param_dict["learning_rate"]),
                      loss=get_loss_fun(param_dict["loss_fun"], from_logits=param_dict["from_logits"]),
                      metrics=get_metrics(param_dict["metrics"]))
    print("====================Model Training====================")
    deepfm_model.fit(train_db, epochs=param_dict["epoch_num"])
    print("====================Model Evaluating====================")
    deepfm_model.evaluate(test_db)
    # save model
    deepfm_model.save(filepath=param_dict["model_dir"], overwrite=True, save_format="tf")
...
if __name__ == "__main__":
    ...
    deepfm_model = tf.keras.models.load_model(param_dict["model_dir"])
    ...
    outputs = deepfm_model.predict(inputs)
    print(outputs)
